<template>
  <div class="hotComments">
    ...
    >
      <van-button class="commentCardSwitch" size="mini" round @click="openDialog"
        ><i class="iconfont icon-ziyuan"></i> 发表我的音乐评论</van-button
      >
      <van-dialog
...
<script>
import {reqSendComment,reqGreatComment,reqSendFloorComment} from '@/request/api/video.js';
...
export default {
  ...
  methods: {
    ...
    async submitComment() {
      ...
      let res = await reqSendComment(data).catch((err) => {
    ...
    async likeCurrentComment(flag, cid) {
      ...
      let res = await reqGreatComment(data);
      console.log(res);
      if (res.data.code == 200) {
        this.$emit("getComment");
      } else {
    ...
    async submitFloorComment() {
      ...
      let res = await reqSendFloorComment(data).catch((err) => {
        // console.log(err.response.data.msg);
        this.$message.error(err.response.data.msg);
      });
...
.van-textarea /deep/ .van-textarea__inner {
  height: 65px !important;
  font-size: 12px;
  resize: none;
}

.van-textarea /deep/ .van-textarea__inner:focus {
  border-color: #aaa;
}

.van-textarea /deep/ .van-input__count {
  transform: scale(0.9);
}
...
.musicCommentArea /deep/ .van-textarea__inner {
...
.commentDialog /deep/ .van-dialog__body {
